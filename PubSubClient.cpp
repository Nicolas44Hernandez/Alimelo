...
PubSubClient::PubSubClient(Adafruit_FONA& client) {
    this->_state = MQTT_DISCONNECTED;
    setClient(client);
    //this->stream = NULL;
}
       ...
       // if (domain != NULL) 
	    {
			...
			result = _client->TCPconnect(this->domain, this->port);
        } /*else {
                ...
                if (type == MQTTPUBLISH) {
                    if (callback != NULL) {
                        ...
                        if ((buffer[0]&0x06) == MQTTQOS1) {
                            ...
                            buffer[2] = (msgId >> 8);
                            buffer[3] = (msgId & 0xFF);
                            _client->TCPsend((char*)buffer,4);
                            lastOutActivity = t;

                        } else {
                            payload = buffer + llen + 3 + tl;
                            payload[len - llen - 3 - tl] = '\0';
...
bool PubSubClient::publish_P(const char* topic, const uint8_t* payload, unsigned int plength, bool retained) {
    ...
    rc += _client->TCPsend((char*)buffer,pos);

    /*for (i=0;i<plength;i++) {
        rc += _client->TCPsend((char)pgm_read_byte_near(payload + i));
    }*/
	rc += _client->TCPsend((char*)payload, plength);
...
bool PubSubClient::write(uint8_t header, uint8_t* buf, uint16_t length) {
    ...
    while((bytesRemaining > 0) && result) {
		...
		result = _client->TCPsend((char*)writeBuf,bytesToWrite);
...
#else	
	lastOutActivity = millis();
	bool result = _client->TCPsend(((char*)buf+(4-llen)),length+1+llen);
	return  result;
...
PubSubClient& PubSubClient::setClient(Adafruit_FONA& client){
    this->_client = &client;
    return *this;
}
